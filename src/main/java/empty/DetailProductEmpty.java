...
public class DetailProductEmpty {
    public List<DetailProduct> getAllProdcutByIdProduct(int id) {
        ...
        try {
            ...
            while (rs.next()) {
                listPro.add( new DetailProduct(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getInt(5),rs.getDouble(6),rs.getDouble(7))) ;
            }
            rs.close();
            s.close();
    ...
    public DetailProduct getProductByIdProduct(int id) {
        ...
        try {
            ...
            ResultSet rs = s.executeQuery();
            if (rs.next()) {
                pro = new DetailProduct(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getInt(5),rs.getDouble(6),rs.getDouble(7)) ;
            }
            rs.close();
            s.close();
